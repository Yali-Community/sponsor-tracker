...
import { readRecords, updateRecords, claimOnce, limit, csvFor, publicRecord, type ReviewRecord } from '../server/store.ts'
  ...
  try {
    ...
    if (req.method === 'GET') {
      if (action === 'public') {
        ...
        return json(res, 200, records.filter(item => item.status === 'approved').map(publicRecord))
      }
      if (action === 'photo') {
        const { records } = await readRecords()
    ...
    if (action === 'submit') {
      ...
      const existing = (await readRecords()).records.find(item => item.request_id === submission.request_id)
      if (existing) {
        if (existing.email !== submission.email || existing.user_id !== submission.user_id) throw new HttpError(409, 'Please reload the form.')
