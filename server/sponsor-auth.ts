...
export function createSponsorChallenge(userId: string, requestId: string, code: string, email = '') {
  return signToken('sponsor-code', 600, { user_id: userId, request_id: requestId, code_hash: hash(`${requestId}:${code}`), account_key: hash(email.toLowerCase()) })
}
export function confirmSponsorCode(challenge: string, code: string) {
  ...
  return signToken('sponsor-verified', 600, { user_id: data.user_id, request_id: data.request_id, account_key: data.account_key })
}
export function requireSponsorVerification(token: unknown, userId: string, requestId: unknown, email = '') {
  const data = typeof token === 'string' ? verifyToken(token, 'sponsor-verified') : null
  if (!data || data.user_id !== userId || data.request_id !== requestId || data.account_key !== hash(email.toLowerCase())) {
