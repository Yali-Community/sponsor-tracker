...
- **Returning sponsors:** check handles as they are entered. Existing usernames show an explicit warning, hide shared profile fields, and require an email code sent to the saved address. Only new payment details and a note are entered. Clear verification when the username changes.
...
The contribution feed uses small avatars and “Name paid ₹amount” sentences, with an @username and relative submission time below. Order it newest first using the CSV's UTC `paid_at` field; refresh labels every 30 seconds. Keep handles unique, case-insensitive, and independent of social IDs. Row hover uses a 3px horizontal nudge and warm background; reduced motion disables the nudge. Transaction IDs remain hidden.
